...
async function loadAndDisplayPersonData(personDir) {
    ...
    createBloodPressureChart(phrData.smartDeviceData.smartBloodPressureMonitor);    
...
function createBloodPressureChart(data) {
    ...
    data.forEach(entry => {
        ...
        const date = new Date(entry.fromDate);
...
function getMonthNumberFromName(monthName){
    ...
    return new Date(`${monthName}`).getMonth() + 1;
}
...
function createWeightChart(data) {
    ...
    data.forEach(entry => {
        const date = new Date(entry.fromDate);
        const weight = parseFloat(entry.weight.split(' ')[0]);
        dataTable.addRow([date, weight]);
        const date2 = new Date(entry.toDate);
        const weight2 = parseFloat(entry.weight.split(' ')[0]);
        dataTable.addRow([date2, weight2]);
    });
...
personSelect.addEventListener("change", (event) => {
    ...
    loadAndDisplayPersonData(`Source data-20230909T223102Z-001/Source data/${selectedDir}`);
});
